...
// GET /api/vendedores
router.get('/', (req, res) => {
  const lista = db.prepare('SELECT * FROM vendedores WHERE ativo = 1 AND tenant_id = ? ORDER BY nome').all(req.tenantId || 1);
...
router.post('/', (req, res) => {
  ...
  const info = db.prepare('INSERT INTO vendedores (tenant_id, nome, telefone, comissao_pct) VALUES (?, ?, ?, ?)')
    .run(req.tenantId || 1, nome, telefone || null, parseFloat(comissao_pct) || 0);
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE vendedores SET nome=?, telefone=?, comissao_pct=? WHERE id=? AND tenant_id=?')
    .run(nome, telefone || null, parseFloat(comissao_pct) || 0, req.params.id, req.tenantId || 1);
  res.json({ ok: true });
});

// DELETE /api/vendedores/:id -> inativa
router.delete('/:id', (req, res) => {
  db.prepare('UPDATE vendedores SET ativo = 0 WHERE id = ? AND tenant_id = ?').run(req.params.id, req.tenantId || 1);
  res.json({ ok: true });
});
