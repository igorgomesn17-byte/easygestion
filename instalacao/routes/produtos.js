...
function proximoCodigo(prefixo) {
  const row = db.prepare(
    "SELECT codigo FROM produtos WHERE codigo LIKE ? ORDER BY id DESC LIMIT 1"
  ).get(prefixo + '%');
  let n = 1;
  if (row) {
...
router.post('/rapido', (req, res) => {
  ...
  const tx = db.transaction(() => {
    const info = db.prepare(`INSERT INTO produtos (codigo, codigo_barras, nome, preco_venda) VALUES (?, ?, ?, ?)`)
      .run(codigo, codigo_barras, nome, preco);
    const produtoId = info.lastInsertRowid;
    const vinfo = db.prepare('INSERT INTO variacoes (produto_id, tamanho, quantidade) VALUES (?, ?, ?)')
      .run(produtoId, tamanho, qtd);
...
router.post('/', (req, res) => {
  ...
  const codigo = proximoCodigo(prefixo);
  const codigo_barras = gerarEAN13();
  ...
  const insertProduto = db.prepare(`
    INSERT INTO produtos (codigo, codigo_barras, nome, categoria, descricao, cor, custo, preco_venda, foto, colecao)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const insertVar = db.prepare('INSERT INTO variacoes (produto_id, tamanho, quantidade) VALUES (?, ?, ?)');
...
router.put('/:id', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    `).run(nome, categoria || null, descricao || null, cor || null,
      parseFloat(custo) || 0, parseFloat(preco_venda) || 0, fotoPath, colecao || null, req.params.id);
