...
router.get('/', (req, res) => {
  ...
  let sql = "SELECT * FROM despesas WHERE substr(data_competencia,1,7) = ? AND recorrente = 0";
  const params = [mes];
...
router.get('/recorrentes', (req, res) => {
  res.json(db.prepare('SELECT * FROM despesas WHERE recorrente = 1 ORDER BY categoria, descricao').all());
});

// GET /api/despesas/a-pagar -> contas a pagar (nao pagas, todas), ordenado por vencimento
router.get('/a-pagar', (req, res) => {
  const rows = db.prepare("SELECT * FROM despesas WHERE pago = 0 AND recorrente = 0 ORDER BY vencimento ASC").all();
  res.json(rows);
});
...
router.post('/', (req, res) => {
  ...
  const info = db.prepare(`
    INSERT INTO despesas (descricao, valor, categoria, tipo, centro, data_competencia, vencimento, data_pagamento, pago, forma_pagamento, recorrente, obs)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(descricao, v, categoria || null, tipo || 'variavel', centro || 'empresa',
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`
    UPDATE despesas SET descricao=?, valor=?, categoria=?, tipo=?, centro=?, data_competencia=?,
      vencimento=?, pago=?, data_pagamento=?, forma_pagamento=?, obs=? WHERE id=?
  `).run(descricao, parseFloat(valor) || 0, categoria || null, tipo || 'variavel', centro || 'empresa',
    data_competencia, vencimento || null, pago ? 1 : 0,
    pago ? (req.body.data_pagamento || hojeLocal()) : null, forma_pagamento || null, obs || null, req.params.id);
  res.json({ ok: true });
});
...
router.post('/:id/pagar', (req, res) => {
  ...
  // busca a despesa
  const d = db.prepare('SELECT * FROM despesas WHERE id = ?').get(despesaId);
  if (***REMOVED***d) return res.status(404).json({ erro: 'Despesa não encontrada' });

  const tx = db.transaction(() => {
    // 1. marca como paga
    db.prepare('UPDATE despesas SET pago = 1, data_pagamento = ?, forma_pagamento = ? WHERE id = ?')
      .run(dataPagamento, formaPagemento, despesaId);

    // 2. se forma de pagamento informada, cria movimento no caixa (sangria)
    if (formaPagemento) {
      // verifica se caixa do dia existe
      const cxDia = db.prepare('SELECT id, aberto FROM caixa_dia WHERE data = ?').get(dataPagamento);

      if (cxDia && cxDia.aberto) {
        // caixa aberto: cria movimento de sangria
        const insMovimento = db.prepare(`
          INSERT INTO caixa_movimentos (data, tipo, valor, forma, motivo, despesa_id)
          VALUES (?, 'sangria', ?, ?, ?, ?)
        `).run(dataPagamento, d.valor, formaPagemento, `Pagamento de ${d.descricao}`, despesaId);
        ...
        if (formaPagemento === 'dinheiro') {
          const sangrias = db.prepare(`
            ...
            WHERE data = ? AND tipo = 'sangria' AND forma = 'dinheiro'
          `).get(dataPagamento).v;
          db.prepare('UPDATE caixa_dia SET sangrias = ? WHERE data = ?').run(sangrias, dataPagamento);
        }
      }
    }
  });
  ...
  const tx = db.transaction(() => {
    for (const m of modelos) {
      ...
      const existe = db.prepare(
        "SELECT id FROM despesas WHERE recorrente_id = ? AND substr(data_competencia,1,7) = ?"
      ).get(m.id, mes);
      if (existe) { jaExistiam++; continue; }
      db.prepare(`
        INSERT INTO despesas (descricao, valor, categoria, tipo, centro, data_competencia, vencimento, pago, recorrente, recorrente_id)
        VALUES (?, ?, ?, 'fixa', ?, ?, ?, 0, 0, ?)
      `).run(m.descricao, m.valor, m.categoria, m.centro || 'empresa', comp,
