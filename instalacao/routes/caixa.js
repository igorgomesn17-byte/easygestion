...
function garantirLinha(data) {
  db.prepare('INSERT OR IGNORE INTO caixa_dia (data) VALUES (?)').run(data);
}
...
function recalcularMovimentos(data) {
  const r = db.prepare(`
    SELECT
...
router.get('/hoje', (req, res) => {
  ...
  let caixa = db.prepare('SELECT * FROM caixa_dia WHERE data = ?').get(data);
  if (***REMOVED***caixa) {
  ...
  Object.assign(caixa, movimentosConta(data));
  ...
  caixa.vendas = db.prepare(`
    ...
    WHERE date(v.data_hora) = ? ORDER BY v.data_hora DESC
  `).all(data);
  // movimentos (abertura/sangria/suprimento) do dia
  caixa.movimentos = db.prepare(
    'SELECT * FROM caixa_movimentos WHERE data = ? ORDER BY criado_em').all(data);
  res.json(caixa);
});
...
router.post('/abrir', (req, res) => {
  ...
  garantirLinha(data);
  const c = db.prepare('SELECT * FROM caixa_dia WHERE data = ?').get(data);
  if (c.fechado) return res.status(400).json({ erro: 'Caixa deste dia já foi fechado.' });
  const tx = db.transaction(() => {
    db.prepare(`UPDATE caixa_dia SET fundo_troco = ?, aberto = 1, aberto_em = datetime('now','localtime') WHERE data = ?`)
      .run(fundo, data);
    db.prepare(`INSERT INTO caixa_movimentos (data, tipo, valor, motivo) VALUES (?, 'abertura', ?, ?)`)
      .run(data, fundo, 'fundo de troco');
  });
  tx();
  res.json({ ok: true });
});
...
router.post('/saldo-conta', (req, res) => {
  ...
  garantirLinha(data);
  db.prepare('UPDATE caixa_dia SET saldo_conta_inicial = ? WHERE data = ?').run(saldo, data);
...
router.post('/sangria', (req, res) => {
  ...
  const tx = db.transaction(() => {
    db.prepare(`INSERT INTO caixa_movimentos (data, tipo, valor, forma, motivo) VALUES (?, 'sangria', ?, ?, ?)`)
      .run(data, valor, forma, req.body.motivo || null);
    recalcularMovimentos(data); // só dinheiro afeta a gaveta
  });
  tx();
  res.json({ ok: true });
});
...
router.post('/saida', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    const info = db.prepare(`INSERT INTO despesas (descricao, valor, categoria, tipo, centro, data_competencia,
                data_pagamento, pago, forma_pagamento, obs)
                VALUES (?, ?, ?, 'variavel', ?, ?, ?, 1, ?, 'Lançada pelo caixa do dia')`)
      .run(descricao, valor, categoria, centro, data.slice(0, 7) + '-01', data, formaDespesa);
    // 2) movimento no caixa (sangria) vinculado à despesa — apagar um apaga o outro
    db.prepare(`INSERT INTO caixa_movimentos (data, tipo, valor, forma, motivo, despesa_id) VALUES (?, 'sangria', ?, ?, ?, ?)`)
      .run(data, valor, forma, descricao, info.lastInsertRowid);
    recalcularMovimentos(data); // só dinheiro afeta a gaveta
  });
  tx();
  res.json({ ok: true });
...
router.delete('/movimento/:id', (req, res) => {
  const m = db.prepare('SELECT * FROM caixa_movimentos WHERE id = ?').get(req.params.id);
  ...
  const tx = db.transaction(() => {
    db.prepare('DELETE FROM caixa_movimentos WHERE id = ?').run(m.id);
    if (m.despesa_id) db.prepare('DELETE FROM despesas WHERE id = ?').run(m.despesa_id); // despesa vinculada some junto
    recalcularMovimentos(m.data);
  });
  tx();
  res.json({ ok: true });
});
...
router.post('/fechar', (req, res) => {
  ...
  db.prepare(`UPDATE caixa_dia SET dinheiro_contado = ?, diferenca = ?, conta_conferida = ?, fechado = 1,
              fechado_em = datetime('now','localtime'), obs = COALESCE(?, obs) WHERE data = ?`)
    .run(contado, diferenca, contaConf, req.body.obs || null, data);
...
router.post('/reabrir', (req, res) => {
  const data = req.body.data || hojeLocal();
  db.prepare(`UPDATE caixa_dia SET fechado = 0, fechado_em = NULL WHERE data = ?`).run(data);
  res.json({ ok: true });
});
...
router.post('/conciliar', (req, res) => {
  ...
  db.prepare(`
    INSERT INTO caixa_dia (data, conciliado, obs) VALUES (?, ?, ?)
    ON CONFLICT(data) DO UPDATE SET conciliado=excluded.conciliado, obs=excluded.obs
  `).run(d, conciliado ? 1 : 0, obs || null);
  res.json({ ok: true });
});
...
router.get('/mes', (req, res) => {
  ...
  const dias = db.prepare('SELECT * FROM caixa_dia WHERE data LIKE ? ORDER BY data').all(prefixo + '%');
