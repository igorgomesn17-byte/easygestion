...
router.post('/', (req, res) => {
  ...
  const cxHoje = db.prepare('SELECT aberto, fechado FROM caixa_dia WHERE data = ?').get(hojeLocal());
  ...
  let comissaoPct = 0;
  if (vendedor_id) {
    const vend = db.prepare('SELECT comissao_pct FROM vendedores WHERE id = ?').get(vendedor_id);
  ...
  const tx = db.transaction(() => {
    ...
    const info = db.prepare(`
      INSERT INTO vendas (cliente_id, vendedor_id, subtotal, desconto, acrescimo, total, forma_pagamento, origem, parcelas,
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(cliente_id || null, vendedor_id || null, subtotal, desc, acrescimo, total, formaPrincipal, origem || 'loja', parcelasPrincipal,
    ...
    const insPgto = db.prepare(`INSERT INTO venda_pagamentos (venda_id, forma, parcelas, valor, taxa_pct, valor_taxa, valor_liquido)
      VALUES (?, ?, ?, ?, ?, ?, ?)`);
    for (const p of partes) {
      insPgto.run(vendaId, p.forma, p.parcelas, p.valor, p.taxaPct, p.valorTaxa, p.liquido);
    }

    // 2. itens + baixa de estoque + movimento
    const insItem = db.prepare(`INSERT INTO venda_itens (venda_id, variacao_id, produto_id, descricao, qtd, preco_unit, custo_unit)
      VALUES (?, ?, ?, ?, ?, ?, ?)`);
    ...
    for (const l of linhas) {
      insItem.run(vendaId, l.variacao_id, l.produto_id, `${l.nome} (${l.tamanho})`, l.qtd, l.preco_unit, l.custo_unit);
    ...
    if (cliente_id) {
      db.prepare(`UPDATE clientes SET total_gasto = total_gasto + ?, num_compras = num_compras + 1, ultima_compra = ?
                  WHERE id = ?`).run(total, hoje, cliente_id);
    }
    ...
    if (trocoForma === 'pix' && trocoVal > 0) {
      db.prepare(`INSERT INTO caixa_movimentos (data, tipo, valor, forma, motivo) VALUES (?, 'suprimento', ?, 'dinheiro', ?)`)
        .run(hoje, trocoVal, `Troco da venda #${vendaId} ficou na gaveta (devolvido por Pix)`);
      db.prepare(`INSERT INTO caixa_movimentos (data, tipo, valor, forma, motivo) VALUES (?, 'sangria', ?, 'pix', ?)`)
        .run(hoje, trocoVal, `Troco da venda #${vendaId} devolvido por Pix`);
      ...
      const m = db.prepare(`SELECT
        ...
        FROM caixa_movimentos WHERE data = ?`).get(hoje);
      db.prepare('UPDATE caixa_dia SET sangrias = ?, suprimentos = ? WHERE data = ?')
        .run(+m.s.toFixed(2), +m.u.toFixed(2), hoje);
    }

    // 4. atualiza caixa do dia
    atualizarCaixaDia(hoje);
...
function atualizarCaixaDia(data) {
  const vendas = db.prepare("SELECT * FROM vendas WHERE date(data_hora) = ?").all(data);
  ...
  const partesDe = db.prepare('SELECT forma, valor FROM venda_pagamentos WHERE venda_id = ?');
  for (const v of vendas) {
    acc.bruto += v.total; acc.liquido += v.valor_liquido; acc.lucro += v.lucro; acc.n++;
    const partes = partesDe.all(v.id);
    if (partes.length) {
  ...
  db.prepare(`
    INSERT INTO caixa_dia (data, total_pix, total_debito, total_credito, total_dinheiro, total_bruto, total_liquido, lucro_dia, num_vendas)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  `).run(data, +acc.pix.toFixed(2), +acc.debito.toFixed(2), +acc.credito.toFixed(2), +acc.dinheiro.toFixed(2),
...
router.get('/', (req, res) => {
  ...
  let sql = `SELECT v.*, c.nome AS cliente_nome, vd.nome AS vendedor_nome
             ...
             LEFT JOIN vendedores vd ON vd.id = v.vendedor_id WHERE 1=1`;
  const params = [];
...
router.patch('/:id/vendedor', (req, res) => {
  ...
  let comissaoPct = 0;
  if (vendedorId) {
    const vend = db.prepare('SELECT comissao_pct FROM vendedores WHERE id = ?').get(vendedorId);
  ...
  const tx = db.transaction(() => {
    db.prepare('UPDATE vendas SET vendedor_id = ?, comissao_valor = ?, lucro = ? WHERE id = ?')
      .run(vendedorId, novaComissao, novoLucro, v.id);
    atualizarCaixaDia(hoje); // lucro do dia muda
  });
  tx();
...
router.get('/:id', (req, res) => {
  ...
  v.itens = db.prepare('SELECT * FROM venda_itens WHERE venda_id = ?').all(v.id);
  v.pagamentos = db.prepare('SELECT forma, parcelas, valor FROM venda_pagamentos WHERE venda_id = ?').all(v.id);
  res.json(v);
});
...
router.delete('/:id', (req, res) => {
  const v = db.prepare('SELECT * FROM vendas WHERE id = ?').get(req.params.id);
  if (!v) return res.status(404).json({ erro: 'Venda nao encontrada' });
  const itens = db.prepare('SELECT * FROM venda_itens WHERE venda_id = ?').all(v.id);
  const hoje = v.data_hora.slice(0, 10);
  const tx = db.transaction(() => {
    for (const it of itens) {
      if (it.variacao_id) {
    ...
    if (v.cliente_id) {
      db.prepare('UPDATE clientes SET total_gasto = total_gasto - ?, num_compras = MAX(num_compras - 1, 0) WHERE id = ?')
        .run(v.total, v.cliente_id);
    }
    db.prepare('DELETE FROM vendas WHERE id = ?').run(v.id);
    atualizarCaixaDia(hoje);
  });
  tx();
  res.json({ ok: true });
