...
router.get('/fluxo', exigirPapel('admin'), (req, res) => {
  ...
  const vendas = db.prepare(`
    ...
    FROM vendas WHERE substr(data_hora,1,7) = ?
  `).get(mes);

  // SAIDAS: despesas do mes (competencia)
  const despPagas = db.prepare(`
    SELECT COALESCE(SUM(valor),0) AS v FROM despesas WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND pago=1
  `).get(mes).v;
  const despApagar = db.prepare(`
    SELECT COALESCE(SUM(valor),0) AS v FROM despesas WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND pago=0
  `).get(mes).v;
  const despEmpresa = db.prepare(`
    SELECT COALESCE(SUM(valor),0) AS v FROM despesas WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND centro='empresa'
  `).get(mes).v;
  const despPessoal = db.prepare(`
    SELECT COALESCE(SUM(valor),0) AS v FROM despesas WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND centro='pessoal'
  `).get(mes).v;
  ...
  const porCategoria = db.prepare(`
    ...
    FROM despesas WHERE substr(data_competencia,1,7)=? AND recorrente=0
...
router.get('/dre', exigirPapel('admin'), (req, res) => {
  ...
  const despFixas = db.prepare(`
    ...
    WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND tipo='fixa' AND centro='empresa'
  `).get(mes).v;
  const despVar = db.prepare(`
    SELECT COALESCE(SUM(valor),0) AS v FROM despesas
    WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND tipo='variavel' AND centro='empresa'
  `).get(mes).v;
  // Pró-labore / retiradas do dono (centro='pessoal') — abatem DEPOIS do operacional
  const proLabore = db.prepare(`
    SELECT COALESCE(SUM(valor),0) AS v FROM despesas
    WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND centro='pessoal'
  `).get(mes).v;
  ...
  if (regimeFiscal === 'mei') {
    ...
    const meiBoleto = db.prepare(`
      SELECT COALESCE(SUM(valor),0) AS v FROM despesas
      WHERE substr(data_competencia,1,7)=? AND recorrente=0 AND categoria='mei'
    `).get(mes).v;
...
router.get('/conciliacao', exigirPapel('admin'), (req, res) => {
  ...
  const cx = db.prepare('SELECT saldo_conta_inicial, conta_conferida FROM caixa_dia WHERE data = ?').get(data) || {};
  ...
  const vendasConta = db.prepare(`
    ...
    FROM vendas WHERE date(data_hora) = ?
  `).get(data);
  ...
  const splitConta = db.prepare(`
    ...
    WHERE date(v.data_hora) = ? AND v.forma_pagamento = 'misto'
  `).get(data);
  ...
  const movs = db.prepare(`
    ...
    WHERE data = ? AND forma <> 'dinheiro' ORDER BY criado_em
  `).all(data);
...
router.post('/conciliacao', exigirPapel('admin'), (req, res) => {
  ...
  if ('saldo_inicial' in req.body) {
    ...
    db.prepare('UPDATE caixa_dia SET saldo_conta_inicial = ? WHERE data = ?').run(s, data);
  }
  if ('conferido' in req.body) {
    const c = req.body.conferido === '' || req.body.conferido == null ? null : parseFloat(req.body.conferido);
    db.prepare('UPDATE caixa_dia SET conta_conferida = ? WHERE data = ?').run(c, data);
  }
  res.json({ ok: true });
});
...
router.get('/fluxo-caixa', exigirPapel('admin'), (req, res) => {
  ...
  for (const v of vendas) {
    const dataPagtos = db.prepare(`
      SELECT forma, parcelas, valor FROM venda_pagamentos WHERE venda_id = ?
    `).all(v.id);
  ...
  const despesas = db.prepare(`
    ...
    WHERE substr(data_competencia,1,7) = ? AND recorrente = 0 AND pago = 1
