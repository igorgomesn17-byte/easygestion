...
/**
 * Cria UM alerta pontual, no momento em que o evento acontece.
 *
 * Existia uma chamada a criarAlerta() em lib/renovacao-scheduler.js — mas a função
 * NUNCA foi escrita aqui. Toda renovação de cliente pagante estourava
 * "criarAlerta is not a function" DEPOIS de já ter criado a cobrança e renovado a
 * data: caía no catch, contava como erro no log, e o alerta nunca era registrado.
 * Nunca deu na vista porque nunca houve renovação real em produção.
 *
 * Diferente de gerarAlertas(), que VARRE a base periodicamente procurando padrões.
 * Este é o registro imediato de um fato ("acabei de gerar uma cobrança").
 *
 * Idempotente: não duplica um alerta do mesmo tipo, pro mesmo tenant, no mesmo dia —
 * o scheduler roda no boot e no horário agendado, e um reboot no dia da cobrança
 * geraria dois alertas idênticos.
 */
function criarAlerta(tenantId, tipo, dados = {}) {
  try {
    const jaExiste = db.prepare(`
      SELECT 1 FROM alertas_clientes
      WHERE tenant_id = ? AND tipo = ? AND resolvido_em IS NULL
    ...
    `).run(
      tenantId,
      tipo,
      Math.floor(dados.dias_atraso || 0),
      dados.valor_em_risco || 0,
      dados.mensagem || tipo
    );
    return { id: Number(info.lastInsertRowid), tenant_id: tenantId, tipo };
  } catch (err) {
    // Um alerta é observabilidade — não pode derrubar a cobrança que acabou de ser
    // criada. Loga e segue.
    console.error(`[ALERTAS] Erro ao criar alerta "${tipo}" p/ tenant ${tenantId}:`, err.message);
    return null;
  }
}
