...
const { acoesDoDia, existeAcaoAtiva, TIPOS_DE_AUSENCIA } = require('./crm');
const { emitirCupom } = require('./cupons');
const { planoDoTenant, temFeature } = require('./planos');

// Conjunto pra lookup O(1) — os tipos cuja janela tem largura (28..32 dias etc.) e
// que por isso se multiplicariam um por dia dentro dela.
const AUSENCIA = new Set(TIPOS_DE_AUSENCIA);
  ...
  db.transaction(() => {
    ...
    const acoes = acoesDoDia(tenantId, data, {
      ...
      emitirCupom: (cli, tipo, { prefixo, pct, dias }) => {
        if (jaExiste.get(tenantId, data, cli.id, tipo)) return null;  // acao de HOJE ja existe: nao emite
        // Tipos de ausencia (reativacao/recompra) tem janela larga: sem esta guarda,
        // um cupom nasceria a cada dia dentro da janela mesmo com a acao anterior ainda
        // pendente. Se ja ha uma ativa, nao emite (o INSERT abaixo tambem sera pulado).
        if (AUSENCIA.has(tipo) && existeAcaoAtiva(tenantId, cli.id, tipo)) return null;
    ...
    for (const a of acoes) {
      // Ausencia (reativacao/recompra) que ja tem uma acao ATIVA em outra data: nao
      // materializa de novo — e' o que impede o mesmo cliente de aparecer 3x dentro da
      // janela de 28..32 dias. O cupom desta rodada (se houve) fica orfao; apaga.
      if (AUSENCIA.has(a.tipo) && existeAcaoAtiva(tenantId, a.cliente_id, a.tipo)) {
        if (a.cupom_id) db.prepare('DELETE FROM crm_cupons WHERE id = ? AND tenant_id = ?').run(a.cupom_id, tenantId);
        continue;
      }
