...
//
// ⚠️ ISTO JÁ QUEBROU A LOJA EM PRODUÇÃO. Os assets da vitrine são servidos com
// `immutable, max-age=1 ano` (server.js) e o `?v=` estava CHUMBADO no HTML. Ao
// deployar um vitrine.js novo, o navegador de quem já tinha visitado continuava
// executando o JS VELHO por um ano — e o JS velho chamava função que o HTML novo
// não tem mais, estourava, e a cliente via "Erro ao carregar a vitrine".
//
// A versão agora sai do mtime dos arquivos: mudou o arquivo, muda a URL, o
// navegador baixa. Ninguém precisa lembrar de incrementar nada.
// ------------------------------------------------------------
let versaoAssets = null;

function versaoDosAssets() {
  if (versaoAssets && EM_PRODUCAO) return versaoAssets;
  const alvos = [
    path.join(__dirname, '..', 'public', 'vitrine', 'js', 'vitrine.js'),
    path.join(__dirname, '..', 'public', 'vitrine', 'js', 'produto.js'),
    path.join(__dirname, '..', 'public', 'vitrine', 'css', 'vitrine.css'),
    path.join(__dirname, '..', 'public', 'css', 'ds.css'),
  ];
  let maior = 0;
  for (const a of alvos) {
    try { maior = Math.max(maior, fs.statSync(a).mtimeMs); } catch (e) { /* arquivo novo ainda não existe */ }
  }
  // Base 36 encurta o timestamp sem perder unicidade.
  versaoAssets = Math.floor(maior / 1000).toString(36);
  return versaoAssets;
}
...
function renderArquivo(nomeArquivo, dados = {}) {
  // `v` sempre disponível nos templates: {{v}} no final da URL do asset.
  return render(lerTemplate(nomeArquivo), { v: versaoDosAssets(), ...dados });
}
