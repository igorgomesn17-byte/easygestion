...
function acharOuCriarConversa(tenantId, { telefone, nome = null, origem = null, canal = 'whatsapp', externalContactId = null }) {
  const t = exigirTenant(tenantId);

  // INSTAGRAM NÃO TEM TELEFONE. A identidade é o IGSID, e casar por ele é a única
  // forma de reconhecer a mesma pessoa voltando. Antes disto `if (!tel) return
  // null` fazia toda conversa do Instagram ser descartada em silêncio — a
  // mensagem chegava no webhook e desaparecia.
  if (canal === 'instagram') {
    if (!externalContactId) return null;
    const ja = db.prepare(
      `SELECT * FROM conversas
        WHERE tenant_id = ? AND arquivada = 0 AND canal = 'instagram' AND external_contact_id = ?
        ORDER BY ultima_interacao DESC LIMIT 1`
    ).get(t, String(externalContactId));
    if (ja) return ja;

    const r = db.prepare(`
      INSERT INTO conversas (tenant_id, canal, external_contact_id, contato_nome, estagio, origem, departamento, usuario_id, ultima_interacao)
      VALUES (?, 'instagram', ?, ?, 'novo', ?, 'c1', ?, datetime('now','localtime'))
    `).run(t, String(externalContactId), nome || null, origem || 'instagram', donoPadraoDe(t, 'c1'));
  ...
  // DEPARTAMENTO (modelo MCC) — decidido AQUI e congelado.
  //
  //   nunca comprou -> c1 (levantada de mao: falta a primeira venda)
  //   ja comprou    -> c2 (a relacao e' dele; esforco menor, ticket maior)
  //
  // Congelar em vez de calcular na leitura: recalcular faria o card pular de fila
  // no instante em que a cliente comprasse — no meio de uma conversa, com o C1
  // ainda escrevendo a resposta. A passagem de bastao acontece na venda, num
  // ponto controlado (`passarParaC2`), nao por efeito colateral de um SELECT.
  const depto = cli && Number(cli.num_compras) > 0 ? 'c2' : 'c1';

  const r = db.prepare(`
    INSERT INTO conversas (tenant_id, cliente_id, canal, telefone, contato_nome, estagio, origem, departamento, usuario_id, ultima_interacao)
  ...
  `).run(
    ...
    origem || null,
    depto,
    // Dono automatico: se ha UMA pessoa naquele departamento, a conversa ja nasce
    // com ela. Com duas ou mais, fica sem dono e a primeira que assumir leva —
    // rodizio automatico exigiria uma regra de justica que ninguem pediu.
    donoPadraoDe(t, depto),
  );

  return db.prepare('SELECT * FROM conversas WHERE id = ?').get(Number(r.lastInsertRowid));
}

// Quem cuida deste departamento? Só resolve quando a resposta é ÓBVIA (uma pessoa
// só com aquele papel). Com duas, devolve null: a conversa entra na fila do
// departamento sem dono, e quem abrir primeiro assume — que é o comportamento
// que `assumir()` já garante sem roubo.
function donoPadraoDe(tenantId, depto) {
  const papel = depto === 'c2' ? 'comercial_2' : 'comercial_1';
  const donos = db.prepare(
    'SELECT id FROM usuarios WHERE tenant_id = ? AND papel = ? AND ativo = 1'
  ).all(tenantId, papel);
  return donos.length === 1 ? donos[0].id : null;
}

// ------------------------------------------------------------
// PASSAGEM DE BASTÃO — a primeira compra move a conversa pro Comercial 2.
// ------------------------------------------------------------
// É o MCC ao pé da letra: o C1 existe pra fazer a PRIMEIRA venda; assim que ela
// acontece, a relação passa pra quem cuida da base. Sem isso o C1 acumula
// carteira e sobra menos tempo pra prospectar — que é o único trabalho dele.
//
// Chamado de dentro da transação da venda. Se a conversa já era do C2, não faz
// nada (a cliente comprou de novo, não é passagem).
function passarParaC2(tenantId, clienteId) {
  const t = exigirTenant(tenantId);
  if (!clienteId) return 0;

  // O dono ANTERIOR é substituído de propósito: manter o C1 como dono de uma
  // conversa que agora é do C2 deixaria o card na fila errada, e com o nome
  // errado no placar. donoPadraoDe resolve na hora se houver um C2 só.
  const r = db.prepare(`
    UPDATE conversas
       SET departamento = 'c2',
           usuario_id = ?,
           estagio = CASE WHEN estagio IN ('novo','falei','catalogo','negociando') THEN 'comprou' ELSE estagio END
     WHERE tenant_id = ? AND cliente_id = ? AND departamento = 'c1' AND arquivada = 0
  `).run(donoPadraoDe(t, 'c2'), t, clienteId);
...
function receberMensagem(tenantId, msg) {
  ...
  // No Instagram a identidade é o IGSID — exigir telefone aqui descartaria toda
  // mensagem que vem de lá, em silêncio.
  if (canal === 'instagram') {
    if (!msg.externalContactId) return { ok: false, erro: 'Contato invalido' };
  } else if (!tel) {
  ...
  try {
    const conversa = acharOuCriarConversa(t, {
      ...
      origem: msg.origem,
      canal,
      externalContactId: msg.externalContactId || null,
    });
    ...
    //
    // No INSTAGRAM não se cria cliente: ela não tem telefone, e cliente sem
    // telefone não serve pra nada no CRM — a régua não alcança, o clube não
    // chama, o vale não vai. A conversa fica sem `cliente_id` até alguém
    // reconhecer quem é e vincular na mão. Criar um cadastro fantasma poluiria a
    // base com gente inalcançável, que é o problema que a migration 040 resolveu.
    let clienteId = conversa.cliente_id;
    if (!clienteId && msg.nome && canal !== 'instagram') {
    ...
    // A RESPOSTA DELA FECHA O CICLO DA REGUA.
    //
    // Ate aqui, status='enviada' significava apenas "alguem clicou no botao" — nao
    // dizia se a mensagem chegou, muito menos se CONVENCEU. Carimbar respondeu_em
    // e' o que transforma a regua de "mandei 40 mensagens" em "40 mandadas, 12
    // responderam" — e o segundo numero e' o unico que diz se o texto funciona.
    //
    // So a acao mais recente e ainda sem resposta e' marcada: se ela respondeu uma
    // vez, as anteriores ja ficaram pra tras, e creditar a resposta a todas inflaria
    // a taxa. Janela de 30 dias porque uma resposta de dois meses depois nao e'
    // resposta AQUELA mensagem — e' assunto novo.
    if (clienteId) {
      db.prepare(`
        UPDATE crm_acoes
           SET respondeu_em = datetime('now','localtime')
         WHERE id = (
           SELECT id FROM crm_acoes
            WHERE tenant_id = ? AND cliente_id = ?
              AND status = 'enviada' AND respondeu_em IS NULL
              AND date(resolvido_em) >= date('now','-30 days')
    ...
    db.exec('COMMIT');
    return {
      ok: true, conversaId: conversa.id, clienteId,
      nova: !conversa.cliente_id,
      // Quem chama (o webhook) usa isto pra decidir se o bot responde: só a
      // PRIMEIRA mensagem de uma conversa parada aciona o bot. Se a lojista já
      // está conversando, o bot entrando no meio seria constrangedor.
      texto,
      conversaNova: !conversa.cliente_id && !conversa.usuario_id,
    };
  } catch (err) {
    db.exec('ROLLBACK');
    throw err;
...
// O bot deve responder esta conversa AGORA?
//
// NÃO responde quando: a lojista já assumiu (usuario_id), ela já respondeu alguma
// vez, ou o bot foi pausado nesta conversa. Bot que fala por cima de um humano que
// já está atendendo é pior que bot nenhum — a cliente vê duas vozes na mesma janela.
//
// `ia_pausada` já existia no schema (sobrou do Inbox removido, nunca usada) e é
// exatamente o campo certo: um clique da lojista cala o bot naquela conversa.
function botDeveResponder(tenantId, conversaId) {
  const t = exigirTenant(tenantId);
  const c = db.prepare('SELECT usuario_id, ia_pausada FROM conversas WHERE id = ? AND tenant_id = ?')
    .get(conversaId, t);
  if (!c) return false;
  if (c.ia_pausada) return false;
  if (c.usuario_id) return false;

  const humano = db.prepare(`
    SELECT 1 FROM mensagens WHERE conversa_id = ? AND direcao = 'enviada' LIMIT 1
  `).get(conversaId);
  return !humano;
}

// Cala o bot nesta conversa — usado quando ele transfere, e pelo botão da tela.
function pausarBot(tenantId, conversaId) {
  const t = exigirTenant(tenantId);
  db.prepare('UPDATE conversas SET ia_pausada = 1 WHERE id = ? AND tenant_id = ?').run(conversaId, t);
}
...
module.exports = {
  ...
  botDeveResponder, pausarBot,
  passarParaC2, donoPadraoDe,
  JANELA_HORAS,
};
