...
function cfgNum(chave, fallback, tenantId = 1) {
  const v = getConfig(chave, null, tenantId);
  return v !== null ? parseFloat(v) : fallback;
}

// Retorna a taxa (%) da forma de pagamento escolhida
function taxaPorForma(forma, parcelas = 1, tenantId = 1) {
  switch (forma) {
...
function sugerirPreco(custo, tenantId = 1) {
...
function resultadoVenda(total, custoTotal, forma, parcelas = 1, comissaoPct = 0, embalagemTotal = 0, freteTotal = 0, tenantId = 1) {
...
function acrescimoParcelamento(total, parcelas, tenantId = 1) {
