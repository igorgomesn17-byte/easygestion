...
function analisarPreco(custo, precoVenda, opc = {}) {
  ...
  const frete = opc.frete != null ? opc.frete : cfgNum('frete_unit', 0);
  const embalagem = opc.embalagem != null ? opc.embalagem : cfgNum('embalagem_unit', 1);
  const impostoPct = cfgNum('imposto_simples', 7.30);
  const comissaoPct = opc.comissaoPct != null ? opc.comissaoPct : cfgNum('comissao_padrao', 5);
  // taxa de referencia para formar/avaliar o preco (default credito a vista)
  const taxaRefPct = opc.taxaPct != null ? opc.taxaPct : cfgNum('taxa_referencia_preco', 3.15);
