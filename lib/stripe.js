...
const { definicaoPlano, normalizarPlano, ehVendavel } = require('./planos');
...
async function criarCheckoutSession(tenantId, tier, ciclo = 'mensal') {
  try {
    ...
    // Allowlist: só plano PÚBLICO passa pelo checkout. O 'interno' (uso do dono) e o
    // 'enterprise' (congelado) não têm preço nem Price ID — sem esta guarda eles
    // morreriam lá embaixo com "Price ID não configurado", que sugere criar o Price
    // no Stripe. A intenção é o oposto: esses planos NÃO se vendem.
    if (!ehVendavel(plano)) throw new Error(`Plano "${plano}" não está à venda — não pode ser contratado pelo checkout.`);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // ⚠️ ESSENCIAL: carimba o tenant_id na SUBSCRIPTION, não só na Session. Os eventos
      // de renovação (invoice.payment_succeeded etc.) recebem a subscription, NÃO a
      // session — sem isto, subscription.metadata vem vazio e a renovação do cliente
      // pagante era ignorada (ele acabava bloqueado como inadimplente). Ver
      // tenantDaSubscription().
      subscription_data: {
        metadata: {
          tenant_id: tenantId,
          plano: plano,
...
async function criarPortalSession(tenantId) {
  ...
  if (!tenant || !tenant.stripe_customer_id) {
    // Estado ESPERADO (trial/cadastro manual sem checkout), não é erro de Stripe.
    // Marca com code pra rota devolver 409 + redirect em vez de 500, sem logar como falha.
    const e = new Error('Cliente Stripe não encontrado para este tenant');
    e.code = 'SEM_CUSTOMER';
    throw e;
  }
...
// Descobre QUAL tenant é o dono de uma subscription do Stripe, com dois caminhos:
//
//   1. subscription.metadata.tenant_id — o jeito direto, mas so existe nas assinaturas
//      criadas DEPOIS de 14/07/2026 (quando o checkout passou a carimbar isso via
//      subscription_data.metadata + o backfill no checkout.session.completed).
//   2. tenants.stripe_customer_id = subscription.customer — o fallback, que funciona
//      pra QUALQUER assinatura, inclusive as antigas que nunca terao o metadata.
//
// Por que os dois: as assinaturas que ja existem em producao foram criadas SEM o
// metadata na subscription (ele so ia pra Session e pro Customer). Os eventos de
// dinheiro — invoice.payment_succeeded/failed, subscription.deleted — liam
// subscription.metadata.tenant_id, achavam vazio, e davam `break`. Efeito real: o
// cliente pagava todo mes, a renovacao nunca rodava, e em ~31 dias ele era bloqueado
// como inadimplente. O fallback pelo customer conserta isso pros clientes atuais; o
// metadata conserta pros novos (e e' mais barato: nao precisa buscar o tenant no banco).
function tenantDaSubscription(subscription) {
  const doMeta = parseInt(subscription?.metadata?.tenant_id);
  ...
  if (customerId) {
    const t = db.prepare('SELECT id FROM tenants WHERE stripe_customer_id = ?').get(customerId);
    if (t) return t.id;
  }
  return null;
}
  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        // Um plano não-vendável nunca deveria chegar aqui (criarCheckoutSession barra).
        // Se chegou, o fallback da linha acima leu tenants.plano de um tenant interno
        // e gravaria uma assinatura de R$ 0,00 — pior, marcaria em_teste=0. Aborta.
        if (!ehVendavel(plano)) {
          console.warn(`[Webhook] Ignorado: checkout de plano não-vendável "${plano}" p/ tenant ${tenantId}`);
          break;
        }
        ...
        // Garante o carimbo na subscription mesmo que subscription_data não tenha
        // pegado (ex.: checkout criado antes deste deploy). É o que os eventos de
        // renovação vão ler depois. Best-effort: se a chamada falhar, o fallback por
        // stripe_customer_id em tenantDaSubscription ainda cobre.
        if (!subscription.metadata?.tenant_id) {
          try {
            await stripe.subscriptions.update(stripeSubscriptionId, {
              metadata: { tenant_id: tenantId, plano, ciclo },
            });
          } catch (e) {
            console.warn(`[Webhook] Não consegui carimbar tenant_id na subscription ${stripeSubscriptionId}: ${e.message}`);
          }
        }
      ...
      case 'invoice.payment_succeeded': {
        ...
        const tenantId = tenantDaSubscription(subscription);

        if (!tenantId) {
          console.warn(`[Webhook] Fatura paga mas tenant não encontrado (sub ${subscription.id}, customer ${subscription.customer})`);
          break;
        }
      ...
      case 'invoice.payment_failed': {
        ...
        if (!tenantId) {
          console.warn(`[Webhook] Fatura falhou mas tenant não encontrado (sub ${subscription.id}, customer ${subscription.customer})`);
          break;
        }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (!tenantId) {
          console.warn(`[Webhook] Assinatura deletada mas tenant não encontrado (sub ${subscription.id}, customer ${subscription.customer})`);
          break;
        }
...
//
// ⚠️ TRIAL VENCIDO NÃO É INADIMPLÊNCIA — e a diferença importa muito pro cliente.
//
// `tenants.status = 'bloqueado'` é o estado de PUNIÇÃO: middleware/seguranca.js
// destrói a sessão e responde "Sua conta foi bloqueada pelo administrador". A pessoa
// nem consegue logar.
//
// Fim de trial é outra coisa: o cliente experimentou o Growth por 14 dias e agora
// precisa ESCOLHER um plano. Ele tem que conseguir entrar e ser levado à tela de
// planos — que é o que validarTenantAtivo já faz (403 + redirecionar:'/planos.html'
// + trial_expirado:true) quando obterStatusAssinatura devolve 'vencida'.
//
// Sem a guarda de em_teste abaixo, o trial vencia e este job o marcava como
// 'bloqueado'. O cliente levava uma acusação de banimento na cara, sem nem ver a
// tela de planos. O oposto do fluxo desenhado: trial → bloqueia (gentil) → escolhe → paga.
async function verificarEBloquearPorAtrasoComStrike(tenantId) {
  try {
    ...
    // Em trial: nunca punir. O portão do fim do trial é o validarTenantAtivo, que
    // deixa entrar e leva pra /planos.html. Quem paga tem em_teste zerado no checkout
    // (lib/stripe.js, checkout.session.completed) e volta a ser elegível a bloqueio.
    if (assinatura.em_teste === 1) return;
