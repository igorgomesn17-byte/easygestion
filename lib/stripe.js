...
// Planos Stripe (Price IDs do Stripe Dashboard)
const PLANOS_STRIPE = {
  mensal: {
    price_id: process.env.STRIPE_PRICE_MENSAL || 'price_1ToiYZJNWhW8rGHJ4WdZHspz',
    nome: 'EasyGestão - Mensal',
    preco: 9990, // R$99,90 em centavos
    periodo: 'mensal',
  },
  anual: {
    price_id: process.env.STRIPE_PRICE_ANUAL || 'price_1ToidmJNWhW8rGHJbDlnzHg7',
    nome: 'EasyGestão - Anual',
    preco: 107880, // R$1.078,80 em centavos
    periodo: 'anual',
  },
};
...
async function criarCheckoutSession(tenantId, tipo_plano) {
  try {
    const customerId = await criarOuBuscarCliente(tenantId);
    const planConfig = PLANOS_STRIPE[tipo_plano];
    if (!planConfig) throw new Error(`Plano inválido: ${tipo_plano}`);

    // Usar price_id direto (configurado no Stripe Dashboard)
    // Ramificar por tipo de plano (mensal = subscription, anual = payment)
    const isAnnual = tipo_plano === 'anual';

    // Ambos os planos usam mode:subscription para funcionar com parcelamento nativo do Stripe
  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        const tipoPlano = session.metadata?.tipo_plano || 'mensal';

        if (!tenantId) throw new Error('Tenant ID não encontrado no webhook');

        const dataInicio = new Date().toISOString().split('T')[0];

        // Buscar subscription do Stripe (ambos os planos agora usam mode:subscription)
        const subscription = await stripe.subscriptions.retrieve(session.subscription);
        const stripeSubscriptionId = subscription.id;

        // Determinar intervalo de renovação conforme o tipo de plano
        const daysToAdd = tipoPlano === 'anual' ? 365 : 30;
        ...
        // Calcular valor mensal corretamente (plano anual deve ser desdobrado em 12 meses)
        const valorMensalReal = tipoPlano === 'anual'
          ? Math.round((PLANOS_STRIPE[tipoPlano].preco / 100) / 12 * 100) / 100 // R$ 1.078,80 / 12 = R$ 89,90
          : PLANOS_STRIPE[tipoPlano].preco / 100;

        if (existing) {
          // Atualizar: marcar como pago (em_teste = 0)
          db.prepare(`
            UPDATE assinaturas
        ...
        console.log(`✅ [Webhook] Assinatura ${existing ? 'atualizada' : 'criada'} para tenant ${tenantId} (plano: ${tipoPlano}, renovação em ${daysToAdd}d)`);
        break;
      }
