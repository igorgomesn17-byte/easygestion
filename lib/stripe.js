...
async function criarCheckoutSession(tenantId, tipo_plano) {
  try {
    ...
    // Ramificar payment methods: apenas cartão (sem parcelamento nativa via Stripe em mode:payment)
    // PIX é um método de pagamento single-use, disponível em mode:payment mas não em subscriptions
    const paymentMethods = isAnnual ? ['card'] : ['card'];
  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        // Ramificar por tipo de sessão (subscription vs payment)
        let stripeSubscriptionId = null;
        let daysToAdd = 30;

        if (session.mode === 'subscription') {
          // ✅ Plano mensal: buscar subscription do Stripe
          const subscription = await stripe.subscriptions.retrieve(session.subscription);
          stripeSubscriptionId = subscription.id;
          daysToAdd = 30;
        } else if (session.mode === 'payment') {
          // ✅ Plano anual: sem subscription, renovação manual em 365 dias
          stripeSubscriptionId = null;
          daysToAdd = 365;
        } else {
          throw new Error(`Modo de sessão desconhecido: ${session.mode}`);
        }
