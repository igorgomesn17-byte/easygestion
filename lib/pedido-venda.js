...
function converter(tenantId, pedidoId, { forma = 'pix', origem = 'vitrine' } = {}) {
  ...
  try {
    ...
    // PASSAGEM DE BASTÃO (MCC): comprou pela primeira vez → a conversa dela sai
    // do Comercial 1 e vai pro Comercial 2. Fora da transação porque é ajuste de
    // atendimento, não de dinheiro: falhar aqui não pode desfazer uma venda paga.
    if (pedido.cliente_id) {
      try {
        require('./conversas').passarParaC2(t, pedido.cliente_id);
      } catch (err) {
        require('./logger').warn('[PEDIDO->VENDA] venda ok, conversa nao passou pro C2:', err.message);
      }
    }
