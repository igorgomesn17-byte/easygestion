...
function mensagemDe(tenantId, tipo, cli, extras = {}) {
  ...
  return { texto: interpolar(tpl.texto, vars), cupom: tpl.cupom || null };
}
...
function acoesDoDia(tenantId, hoje, opc = {}) {
  ...
  const clientes = db.prepare(
    'SELECT * FROM clientes WHERE tenant_id = ? AND arquivado = 0 AND nao_perturbe = 0'
  ).all(t);
...
function segmentarRFM(tenantId, hoje) {
  ...
  const clientes = db.prepare(
    'SELECT * FROM clientes WHERE tenant_id = ? AND num_compras > 0 AND arquivado = 0'
  ).all(t);
