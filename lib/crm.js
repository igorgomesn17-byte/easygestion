...
const { DEFAULT_TEMPLATES, TEMPLATES_SEGMENTO, VARIANTES_SEGMENTO, ROTULOS, interpolar } = require('./crm-templates');
...
function templateDe(tenantId, tipo) {
  ...
  if (!row) return { ...base, ativo: 1, __editado: false };
  ...
  // `__editado` marca que a LOJISTA escreveu o texto deste tipo. mensagemDe() usa
  // isso pra NAO trocar a mensagem dela por uma variante de segmento: quem editou
  // espera ver exatamente o que escreveu. (So o texto conta — mexer no pct do cupom
  // nao deveria congelar a redacao.)
  merged.__editado = !!(row.texto && String(row.texto).trim() !== '');
  return merged;
}

// Prazo de troca ja por extenso ("3 dias úteis"), pronto pra entrar na frase.
// Vive aqui e nao no template porque a loja configura o NUMERO, e a mensagem
// precisa do texto flexionado — 1 dia / 2 dias, útil / úteis.
function trocaPrazo(tenantId) {
  const t = exigirTenant(tenantId);
  const n = cfgNum(t, 'troca_prazo_dias', 3);
  const uteis = getConfig('troca_prazo_uteis', '1', t) === '1';
  if (!(n > 0)) return 'combinado na loja';
  return `${n} ${n === 1 ? 'dia' : 'dias'}${uteis ? (n === 1 ? ' útil' : ' úteis') : ''}`;
}
...
function varsDe(tenantId, cli, extras = {}) {
  ...
  return {
    ...
    selos: s.noCiclo,
    faltam: s.faltam,
    troca_prazo: trocaPrazo(t),
    // {ps} nasce VAZIO: so o gatilho do dia 0 preenche (com o trecho do clube).
    // Sem este default, o preview do DIA1 na tela de edicao mostraria "{ps}" cru,
    // porque interpolar() deixa chave desconhecida literal de proposito.
    ps: '',
    ...extras,
  };
}
...
// ---------- Modulacao por segmento (RFM temperando a regua) ----------
//
// A regua decide QUANDO falar; o RFM decide COMO falar e QUANTO oferecer.
//
// MULTIPLICADOR DE DESCONTO. Decisao do Igor (18/07/2026): mais desconto pra quem
// vale mais. O racional e' de margem, nao de generosidade — perder uma campea custa
// muito mais que perder quem comprou uma blusa uma vez, entao vale pagar mais caro
// pra trazer a campea de volta. E queimar 25% com quem gasta R$50 destroi a margem
// sem trazer resultado.
//
// Aplicado SO sobre o pct que o template ja definiu (0% continua 0%: gatilho sem
// cupom nao ganha cupom por causa do segmento). E o resultado passa pelo teto de
// MAX_PCT (lib/cupons.js barra > 50%) — aqui o teto e' menor ainda, por seguranca.
const MULT_SEGMENTO = {
  campeas:     1.25,  // a perda mais cara da loja: vale pagar mais pra trazer de volta
  risco:       1.25,  // gastava bem e esta indo embora — e' agora ou nunca
  perdidas:    1.15,  // alto valor, sumiu faz tempo: ultima cartada
  fieis:       1.0,
  novas:       1.0,
  promissoras: 1.0,
  atencao:     1.0,
  hibernando:  0.75,  // baixo valor e inativa: nao queimar margem aqui
};
const PCT_TETO_REGUA = 35;   // teto proprio da regua, abaixo do MAX_PCT global (50)

// Ajusta o pct do cupom pelo segmento. Sem segmento (ou segmento desconhecido),
// devolve o pct original — a regua continua funcionando exatamente como antes.
function pctDoSegmento(pctBase, segmento) {
  const base = Number(pctBase) || 0;
  if (base <= 0) return base;                 // gatilho sem cupom NAO ganha cupom
  const mult = MULT_SEGMENTO[segmento];
  if (!mult || mult === 1) return base;
  return Math.min(Math.round(base * mult), PCT_TETO_REGUA);
}

// Renderiza a mensagem. `extras.segmento` (quando vem) escolhe a VARIANTE do texto e
// modula o pct do cupom.
//
// PRECEDENCIA DO TEXTO (o que ganha de quem):
//   1. Template que a LOJISTA editou   <- sempre ganha; ela escreveu, e a palavra e' dela
//   2. Variante do segmento            <- so quando ela nao editou aquele tipo
//   3. Template padrao do tipo         <- o comportamento de sempre
// Sobrescrever o texto que a lojista escreveu seria o pior tipo de "inteligencia":
// ela edita, salva, e o sistema manda outra coisa sem avisar.
function mensagemDe(tenantId, tipo, cli, extras = {}) {
  const tpl = templateDe(tenantId, tipo);
  if (!tpl) return null;

  const seg = extras.segmento || null;
  const lojaEditouEsteTipo = tpl.__editado === true;
  const variante = seg ? VARIANTES_SEGMENTO[`${tipo}:${seg}`] : null;
  const texto = (!lojaEditouEsteTipo && variante) ? variante : tpl.texto;

  // O pct modulado tem que aparecer TAMBEM no texto ({cupom_pct}), senao a mensagem
  // promete 20% e o caixa aceita 25% (ou pior, o contrario).
  const pctFinal = pctDoSegmento(tpl.cupom_pct, seg);

  // {cupom_validade} PRECISA de um default aqui, junto dos outros campos de cupom.
  //
  // Sem ele o texto saia com "{cupom_validade}" CRU pra cliente sempre que a mensagem
  // fosse renderizada sem o cupom nominal emitido — que e' o caso de todo preview
  // (tela de edicao, campanha por segmento) e de qualquer chamada que nao passe pelo
  // scheduler. A lojista lia "Vale até {cupom_validade}" e mandava assim.
  //
  // O fallback e' a validade CALCULADA a partir de cupom_dias: quando o codigo real
  // nascer, `extras.cupom_validade` sobrescreve com a data verdadeira do cupom.
  let validadeFallback = '';
  if (tpl.cupom_dias > 0) {
    const dv = new Date();
    dv.setDate(dv.getDate() + Number(tpl.cupom_dias));
    validadeFallback = String(dv.getDate()).padStart(2, '0') + '/' + String(dv.getMonth() + 1).padStart(2, '0');
  }

  const vars = varsDe(tenantId, cli, {
    cupom: tpl.cupom || '', cupom_pct: pctFinal || '', cupom_dias: tpl.cupom_dias || '',
    cupom_validade: validadeFallback,
    ...extras,
  });
  return {
    texto: interpolar(texto, vars),
    cupom: extras.cupom || tpl.cupom || null,
    cupom_prefixo: tpl.cupom || null,
    cupom_pct: pctFinal || null,
...
function acoesDoDia(tenantId, hoje, opc = {}) {
  ...
  const montar = (cli, tipo, prioridade, detalhe, extrasIn = {}, meta = {}) => {
    // O segmento RFM vive em `meta` (ele e' metadado da acao), mas mensagemDe precisa
    // dele em `extras` pra escolher a variante do texto e modular o pct do cupom.
    // Fundir aqui liga o RFM a TODOS os gatilhos de uma vez, sem tocar em cada
    // chamada de montar() espalhada abaixo.
    const extras = meta.segmento ? { ...extrasIn, segmento: meta.segmento } : extrasIn;
  ...
  for (const cli of clientes) {
    ...
    if (dias === 0) {
      // Comprou hoje: UMA mensagem so, com o clube acoplado no fim.
      //
      // Antes daqui saiam DUAS mensagens no mesmo dia (DIA1 + a do clube), e o total
      // do ciclo era 5 contatos em 10 dias por uma compra — nenhum pedido por ela.
      // O risco disso nao e' converter pouco, e' ganhar bloqueio: cliente que bloqueia
      // leva embora o canal inteiro, e nao existe cupom que traga de volta.
      //
      // O clube vira um P.S. no fim do DIA1 (`extras.ps`, texto curto) em vez de um
      // disparo proprio. Loja que desligou o DIA1 continua recebendo os templates de
      // clube pela tela de edicao — eles nao sumiram, so pararam de disparar sozinhos.
      let ps = '';
      let detalheClube = '';
      if (ativo) {
        if (s.noCiclo === 0 && s.premiosGanhos > 0) {
          ps = `\n\nE tem mais: você completou o cartão do ${c.nome}! São R$ ${c.valorPremio} de vale esperando na sua próxima compra. 🎊`;
          detalheClube = ` • completou ${c.totalSelos} selos!`;
        } else if (cli.num_compras === 1) {
          ps = `\n\nAh, você entrou no ${c.nome}: a cada R$ ${c.valorSelo} em compras você ganha 1 selo, e ${c.totalSelos} selos viram R$ ${c.valorPremio} pra gastar aqui. O primeiro já é seu. ✨`;
          detalheClube = ' • primeira compra';
        } else {
          ps = `\n\nVocê já tem ${s.noCiclo} selos no ${c.nome}, faltam ${s.faltam} pra ganhar R$ ${c.valorPremio}. 🌟`;
          detalheClube = ` • ${s.noCiclo} selos, faltam ${s.faltam}`;
        }
      }
      add(montar(cli, 'DIA1', 1, `Comprou hoje${detalheClube}`, { ps }, meta));
    }
    // Os gatilhos de dia EXATO (3, 7, 10) so funcionam porque o scheduler roda
    // todo dia e materializa. Sem ele, um dia sem abrir a tela perdia o gatilho.
    //
    // O GOOGLE saiu do dia 5 pro dia 7: no dia 5 ele caia na sombra do POS_VENDA_1
    // (dia 3) — dois pedidos em 48h, o segundo pedindo favor. No dia 7 ele respira,
    // e ja chega depois de o prazo de troca ter passado sem problema (o silencio
    // dela nesses dias e' o sinal de que deu tudo certo).
    else if (dias === 3)  add(montar(cli, 'POS_VENDA_1', 2, 'Comprou há 3 dias', { dias }, meta));
    else if (dias === 7)  add(montar(cli, 'GOOGLE',      2, 'Comprou há 7 dias', { dias }, meta));
  ...
  // --- CARRINHO ABANDONADO ---
  //
  // O contato mais QUENTE que existe: ela escolheu as pecas, viu o total, e parou.
  // Nao sumiu — esta decidindo agora. Por isso a prioridade 1 (acima da campea que
  // sumiu ha 40 dias) e por isso NAO entra em TIPOS_DE_AUSENCIA: nao ha ausencia
  // pra lamentar, e uma compra nao invalida o gatilho de forma retroativa.
  //
  // Sai de `vitrine_pedidos`, nao de tabela nova: pedido 'novo' com telefone e
  // algumas horas de idade JA E' um carrinho abandonado. O dado sempre esteve la.
  //
  // Nao pede telefone de `clientes` porque na maioria das vezes ela ainda nem e'
  // cliente — o pedido da vitrine e' anonimo ate alguem vincular.
  const horasCarrinho = cfgNum(t, 'crm_carrinho_horas', 4);
  if (horasCarrinho > 0) {
    let pedidos = [];
    try {
      pedidos = db.prepare(`
        SELECT p.id, p.codigo, p.cliente_id, p.cliente_nome, p.cliente_tel, p.total, p.qtd_itens
          FROM vitrine_pedidos p
         WHERE p.tenant_id = ?
           AND p.status = 'novo'
           AND p.cliente_tel IS NOT NULL AND p.cliente_tel <> ''
           -- janela fechada: passou de 48h, a mensagem chega tarde demais pra ser
           -- util e vira incomodo ("aquele carrinho de terca").
           AND p.criado_em <= datetime('now','localtime', ?)
           AND p.criado_em >= datetime('now','localtime','-48 hours')
         ORDER BY p.total DESC
         LIMIT 50
      `).all(t, `-${horasCarrinho} hours`);
    } catch (_) {
      // Loja sem a tabela da vitrine (banco antigo) simplesmente nao tem o gatilho.
      pedidos = [];
    }

    // A ficha da cliente, quando ela ja e' cliente: e' dela que sai o nome certo,
    // o segmento e o telefone confiavel. `clientes` ja foi carregado la em cima
    // com o filtro de tenant/arquivado/nao-perturbe — reusar respeita tudo isso
    // de graca, inclusive o opt-out de LGPD.
    const fichaPorId = new Map(clientes.map((c) => [c.id, c]));

    for (const p of pedidos) {
      // Sem cliente vinculada nao da' pra materializar em crm_acoes (a FK exige
      // cliente_id). O pedido anonimo aparece na aba de Pedidos, que e' onde ele
      // pertence — e vira acao no dia em que a lojista vincular a cliente.
      const cli = p.cliente_id && fichaPorId.get(p.cliente_id);
      if (!cli) continue;

      const total = Number(p.total || 0);
      const valor = total.toFixed(2).replace('.', ',');
      add(montar(cli, 'CARRINHO', 1,
        `Montou carrinho de R$ ${valor} e não finalizou`,
        { valor, pedido: p.codigo, itens: p.qtd_itens },
        { segmento: rfmDoCliente(cli, hoje).segmento }));
    }
  }

  // --- CADENCIA SEMANAL DE CATALOGO ---
  //
  // Diferente de TODOS os outros gatilhos: nao reage a comportamento nenhum. E' o
  // "toda semana a loja precisa receber o catalogo" — o que sustenta o giro no
  // atacado. Dispara por CALENDARIO (dia da semana configurado), pra base inteira.
  //
  // Desligado por padrao (0 = nunca). Ligar sem a lojista pedir mandaria mensagem
  // semanal pra base inteira de quem nunca pediu isso — o caminho mais rapido pra
  // virar spam e derrubar um numero nao-oficial.
  //
  // 1=segunda ... 7=domingo (ISO). getDay() do JS e' 0=domingo, entao o domingo
  // vira 7 — sem isso, "1" pegaria domingo e a lojista mandaria no dia errado.
  const diaSemana = cfgNum(t, 'crm_catalogo_dia', 0);
  if (diaSemana >= 1 && diaSemana <= 7) {
    const jsDay = new Date(hoje + 'T12:00:00').getDay();   // meio-dia evita fuso virar o dia
    const isoDay = jsDay === 0 ? 7 : jsDay;
    if (isoDay === diaSemana) {
      for (const cli of clientes) {
        // So quem ja comprou alguma vez. Mandar catalogo semanal pra quem nunca
        // comprou e' prospeccao disfarcada de relacionamento — e prospeccao tem
        // lugar proprio (o kanban), com mensagem propria.
        if (!cli.num_compras) continue;
        add(montar(cli, 'CATALOGO_SEMANAL', 4, 'Novidades da semana', {},
          { segmento: rfmDoCliente(cli, hoje).segmento }));
      }
    }
  }
...
// PERIODO DE SILENCIO. Contatar (ou ignorar) resolve o gatilho por um tempo — nao
// so' pra sempre, nao so' por hoje.
//
// O BUG QUE ISTO CONSERTA (visto em producao 18/07/2026): a lojista mandou a
// mensagem de reativacao, a acao virou 'enviada' — e no dia seguinte, as 06:00, o
// scheduler recriou a MESMA acao pra MESMA cliente, porque ela continuava dentro da
// janela de 28..32 dias e 'enviada' nao contava como "ativa". Ela reapareceu na fila
// como se nunca tivesse sido contatada. Mandar de novo no dia seguinte nao e'
// insistencia: e' a loja parecendo desorganizada (ou pior, spam).
const DIAS_SILENCIO = 45;

function existeAcaoAtiva(tenantId, clienteId, tipo) {
  const t = exigirTenant(tenantId);
  return !!db.prepare(
    `SELECT 1 FROM crm_acoes
     WHERE tenant_id = ? AND cliente_id = ? AND tipo = ?
       AND (
         -- ainda na fila, esperando a lojista
         status IN ('pendente','adiada')
         -- ou ja resolvida ha pouco: contato recente silencia o mesmo gatilho.
         -- 'obsoleta' NAO entra: ela virou obsoleta porque a cliente COMPROU, e a
         -- compra reinicia o ciclo — se sumir de novo, a regua deve falar de novo.
         OR (status IN ('enviada','ignorada') AND date(data) >= date('now', ?))
       )
     LIMIT 1`
  ).get(t, clienteId, tipo, `-${DIAS_SILENCIO} days`);
}
...
// ============================================================
// REESCREVER A FILA APOS EDITAR UM TEMPLATE
//
// A regua CONGELA a mensagem no momento da geracao (06:00): o texto vai interpolado
// pra crm_acoes e fica esperando. Isso e' o que faz o gatilho de dia exato nao se
// perder — mas cria uma armadilha de confianca: a lojista edita a mensagem, ve
// "salvo", e a fila continua mostrando o texto ANTIGO. O sistema parece quebrado
// mesmo funcionando como desenhado. (Bug de campo, 24/07/2026: o link do Google que
// ela colou no template nao aparecia em nenhum dos 4 contatos ja na fila.)
//
// Entao editar o template REESCREVE as acoes pendentes daquele tipo.
//
// O QUE NAO SE MEXE:
//   - acoes ja ENVIADAS/ignoradas: historico e' historico. O que foi mandado foi
//     mandado, e reescrever apagaria a prova do que a cliente recebeu.
//   - o CUPOM NOMINAL: o codigo (VOLTE20-K3P9) ja foi emitido e pode ja estar na mao
//     da cliente. Re-renderizamos passando o MESMO codigo — nunca gerando outro.
//   - o pct do cupom ja emitido: quem manda e' a linha em crm_cupons, nao o template
//     novo. Mudar o desconto de um cupom que ja existe seria mexer em dinheiro
//     prometido.
//
// LIMITE CONHECIDO (nao e' bug, nao "conserte"): alguns tipos ganham trechos que
// mensagemDe() sozinho nao reproduz, porque a condicao vive no acoesDoDia — o caso
// real e' o DIA1, que leva um P.S. do clube QUANDO e' a primeira compra da cliente.
// Regerar essa mensagem fora do contexto apagaria o P.S. Por isso a reescrita roda
// SO quando a lojista edita aquele tipo (ai ela quer o texto novo mesmo), e nunca
// como varredura geral da fila.
function reescreverAcoesPendentes(tenantId, tipo) {
  const t = exigirTenant(tenantId);

  const acoes = db.prepare(`
    SELECT a.id, a.cliente_id, a.segmento, a.cupom, a.detalhe,
    ...
    WHERE a.tenant_id = ? AND a.tipo = ? AND a.status IN ('pendente','adiada')
  `).all(t, tipo);
  if (!acoes.length) return 0;

  const upd = db.prepare('UPDATE crm_acoes SET mensagem = ? WHERE id = ? AND tenant_id = ?');
  let n = 0;

  db.transaction(() => {
    for (const a of acoes) {
      const cli = db.prepare('SELECT * FROM clientes WHERE id = ? AND tenant_id = ?').get(a.cliente_id, t);
      if (!cli) continue;

      const extras = {};
      if (a.segmento) extras.segmento = a.segmento;
      // Preserva o codigo JA emitido (e a validade que ja esta valendo).
      if (a.cupom) {
        extras.cupom = a.cupom;
        if (a.cupom_validade) {
          extras.cupom_validade = a.cupom_validade.slice(8, 10) + '/' + a.cupom_validade.slice(5, 7);
        }
        if (a.cupom_pct) extras.cupom_pct = a.cupom_pct;
      }
      // `dias` aparece em varios templates ("{dias} dias sem comprar"). O valor real
      // foi congelado no detalhe; extrai de la pra nao renderizar "undefined dias".
      const m = /(\d+)\s*dias?/i.exec(a.detalhe || '');
      if (m) extras.dias = Number(m[1]);

      const nova = mensagemDe(t, tipo, cli, extras);
      if (!nova || !nova.texto) continue;   // tipo desligado: deixa como esta
      upd.run(nova.texto, a.id, t);
      n++;
    }
  })();
...
module.exports = {
  NOME_BALCAO, clienteBalcao, reescreverAcoesPendentes,
  ...
  TIPOS_DE_AUSENCIA, existeAcaoAtiva, obsoletarAcoesDeAusencia, DIAS_SILENCIO,
  MULT_SEGMENTO, PCT_TETO_REGUA, pctDoSegmento,
};
