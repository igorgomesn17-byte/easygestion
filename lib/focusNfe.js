...
function montarItens(venda, tenantId = 1) {
  ...
  const ncmPadrao   = getConfig('nfce_ncm_padrao', '61099000', tenantId) || '61099000';
  const cfopPadrao  = getConfig('nfce_cfop_padrao', '5102', tenantId) || '5102';
  const csosnPadrao = getConfig('nfce_csosn_padrao', '102', tenantId) || '102';
  const origemProd  = getConfig('nfce_origem_padrao', '0', tenantId) || '0';
  const unidade     = getConfig('nfce_unidade_padrao', 'UN', tenantId) || 'UN';
...
function montarPayloadNfce(venda, ref, tenantId = 1) {
  const ambiente = getConfig('nfce_ambiente', 'homologacao', tenantId);
  const itens = montarItens(venda, tenantId);
  ...
  const payload = {
    ...
    // Emitente (a loja) — vem da config (validado na rota como obrigatório)
    cnpj_emitente: digitos(getConfig('loja_cnpj', '', tenantId)),
    nome_emitente: getConfig('loja_razao_social', '', tenantId) || getConfig('loja_nome', '', tenantId),
    nome_fantasia_emitente: getConfig('loja_nome_fantasia', '', tenantId),
    inscricao_estadual_emitente: digitos(getConfig('loja_ie', '', tenantId)),
    logradouro_emitente: getConfig('loja_endereco', '', tenantId),
    municipio_emitente: getConfig('loja_cidade', '', tenantId),
    uf_emitente: getConfig('loja_uf', 'BA', tenantId),
    cep_emitente: digitos(getConfig('loja_cep', '', tenantId)),
    codigo_municipio_emitente: getConfig('loja_codigo_municipio', '', tenantId),
...
// Aceita token explícito (do cliente) ou usa o do .env (fallback para admin)
async function focusRequest(ambiente, metodo, caminho, corpo, tokenExplicito = null) {
  const token = tokenExplicito || FOCUS.tokenDe(ambiente);
  if (!token) {
    const err = new Error(`Integração NFC-e não configurada (falta o token de ${ambiente})`);
    err.status = 503;
    throw err;
  }
  const url = FOCUS.urlDe(ambiente) + caminho;
  const auth = Buffer.from(token + ':').toString('base64');
  console.log(`[FOCUS] Enviando ${metodo} para ${url}`);
  ...
  try {
    ...
    console.log(`[FOCUS] Resposta recebida: status ${resp.status}`);
    let dados = null;
    try { dados = await resp.json(); } catch (e) { dados = null; }
    if (resp.status !== 201) {
      console.error(`[FOCUS] Erro ${resp.status}:`, JSON.stringify(dados));
    }
    return { httpStatus: resp.status, dados };
  } catch (e) {
    console.error(`[FOCUS] Erro na requisição:`, e.message, e.code);
    throw e;
  }
}
...
// tokenCliente: token explícito do cliente (se houver). Senão usa o do .env
async function emitirNfce(venda, ref, tenantId = 1, tokenCliente = null) {
  const { payload, ambiente, valorTotal } = montarPayloadNfce(venda, ref, tenantId);

  // Debug: logar o payload pra identificar campos faltando/inválidos
  console.log('[NFC-e] Emitindo nota fiscal:', {
    ref,
    ambiente,
    vendaId: venda.id,
    valorTotal,
    ...
    formas_pagamento: payload.formas_pagamento?.length || 0,
  });

  console.log('[NFC-e] ANTES DE CHAMAR focusRequest');
  const { httpStatus, dados } = await focusRequest(ambiente, 'POST', `/v2/nfce?ref=${encodeURIComponent(ref)}`, payload, tokenCliente);
  console.log('[NFC-e] DEPOIS DE focusRequest, httpStatus:', httpStatus);
  const info = interpretarRetorno(dados || {});

  // Debug: logar resposta da Focus
  if (httpStatus !== 201) {
    console.error('[NFC-e] Erro 400+:', {
      httpStatus,
      dados,
    });
  }
...
// GET /v2/nfce/REF
async function consultarNfce(ref, ambiente, tokenCliente = null) {
  const { httpStatus, dados } = await focusRequest(ambiente, 'GET', `/v2/nfce/${encodeURIComponent(ref)}`, null, tokenCliente);
...
async function cancelarNfce(ref, ambiente, justificativa, tokenCliente = null) {
  const { httpStatus, dados } = await focusRequest(ambiente, 'DELETE', `/v2/nfce/${encodeURIComponent(ref)}`, { justificativa }, tokenCliente);
  return { httpStatus, retorno: dados, ...interpretarRetorno(dados || {}) };
}
