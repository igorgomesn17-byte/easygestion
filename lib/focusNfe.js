...
function montarItens(venda, tenantId = 1) {
  ...
  const ncmPadrao   = getConfig('nfce_ncm_padrao', '61099000', tenantId);
  const cfopPadrao  = getConfig('nfce_cfop_padrao', '5102', tenantId);
  const csosnPadrao = getConfig('nfce_csosn_padrao', '102', tenantId);
  const origemProd  = getConfig('nfce_origem_padrao', '0', tenantId);
  const unidade     = getConfig('nfce_unidade_padrao', 'UN', tenantId);
...
function montarPayloadNfce(venda, ref, tenantId = 1) {
  ...
  const payload = {
    ...
    nome_emitente: getConfig('loja_razao_social', '', tenantId) || getConfig('loja_nome', 'DS STORE', tenantId),
    nome_fantasia_emitente: getConfig('loja_nome_fantasia', 'DS STORE', tenantId),
    inscricao_estadual_emitente: digitos(getConfig('loja_ie', '', tenantId)),
    logradouro_emitente: getConfig('loja_endereco', '', tenantId),
    municipio_emitente: getConfig('loja_cidade', 'Itabuna', tenantId),
