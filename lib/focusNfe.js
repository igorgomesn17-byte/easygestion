...
const { exigirTenant } = require('./tenant');
...
// Nota fiscal com o CNPJ, a razao social e o CFOP da loja ERRADA seria
// documento fiscal falso. O tenant nao e' opcional aqui.
function montarItens(venda, tenantId) {
  tenantId = exigirTenant(tenantId, 'focusNfe.montarItens');
...
function montarPayloadNfce(venda, ref, tenantId) {
  tenantId = exigirTenant(tenantId, 'focusNfe.montarPayloadNfce');
...
async function focusRequest(ambiente, metodo, caminho, corpo, tokenExplicito = null) {
  ...
  try {
    ...
    if (resp.status !== 201) {
      console.log(`[FOCUS-ERROR] ${resp.status}:`, JSON.stringify(dados, null, 2));
    }
...
async function emitirNfce(venda, ref, tenantId, tokenCliente = null) {
  tenantId = exigirTenant(tenantId, 'focusNfe.emitirNfce');
