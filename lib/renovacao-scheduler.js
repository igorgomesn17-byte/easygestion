...
function procesarRenovacoes() {
  ...
  try {
    ...
    // 1️⃣ Encontrar assinaturas PAGAS que venceram HOJE.
    //
    // ⚠️ em_teste = 0 é ESSENCIAL. O trial é um registro em `assinaturas` com
    // em_teste=1 e data_proxima_renovacao = fim do trial (routes/auth.js). Sem este
    // filtro, no dia 14 o trial casava aqui e o job (a) criava uma cobrança de
    // R$119,90 por um teste grátis que o cliente nem escolheu pagar, e (b) empurrava
    // a data +30 dias — o que QUEBRAVA o bloqueio de fim de trial: obterStatusAssinatura
    // via a data lá na frente e não bloqueava, dando 30 dias extras de Growth grátis.
    //
    // O fluxo desenhado é: trial vence → NINGUÉM renova → obterStatusAssinatura
    // retorna VENCIDA → o cliente é bloqueado e levado à tela de planos → ele ESCOLHE
    // e paga. O checkout marca em_teste=0 (lib/stripe.js), e só AÍ o registro passa a
    // ser elegível pra renovação recorrente aqui.
