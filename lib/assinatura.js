...
// Fonte única de verdade dos tiers (starter/growth/enterprise + preços/limites/features).
...
function obterStatusAssinatura(tenantId) {
  ...
  // Se foi cancelada, só bloqueia quando chegar a data_proxima_renovacao
  // Até lá, cliente continua com acesso
  if (assinatura.cancelada_em && dataVencimento <= hoje) {
    return {
      status: ESTADOS_ASSINATURA.CANCELADA,
      motivo: `Cancelada em ${assinatura.cancelada_em}. Acesso encerrado.`,
      bloqueado: true,
      dataVencimento,
    };
  }
...
function criarAssinatura(tenantId, plano = 'starter', dataInicio = null) {
  const dataI = dataInicio || new Date().toISOString().split('T')[0];
  const planoKey = normalizarPlano(plano);
  const planoConfig = definicaoPlano(planoKey);
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `).run(tenantId, planoKey, planoConfig.preco_mensal, dataI, dataProxRenovacaoStr);

  return {
    id: result.lastInsertRowid,
    tenant_id: tenantId,
    plano: planoKey,
    valor_mensal: planoConfig.preco_mensal,
    data_inicio: dataI,
...
// Incrementa dias conforme o plano (mensal/basico = +30d, anual = +365d)
function renovarAssinatura(tenantId) {
  ...
  // Determinar incremento de dias baseado no plano
  const diasIncremento = assinatura.plano === 'anual' ? 365 : 30;

  // Próxima renovação = data_proxima_renovacao + dias_incremento
  const dataProxRenovacao = new Date(assinatura.data_proxima_renovacao);
  dataProxRenovacao.setDate(dataProxRenovacao.getDate() + diasIncremento);
  const dataProxRenovacaoStr = dataProxRenovacao.toISOString().split('T')[0];
...
// ✅ Cancelar assinatura
// Cancelamento é efetivo a partir da próxima renovação (30 dias após último pagamento)
function cancelarAssinatura(tenantId, motivo = 'Cancelamento do cliente') {
  ...
  // Calcular data de cancelamento efetivo (próxima renovação)
  const dataUltimoPagamento = new Date(assinatura.data_proxima_renovacao);
  dataUltimoPagamento.setDate(dataUltimoPagamento.getDate() - 30);
  const dataCancelamentoEfetivo = new Date(assinatura.data_proxima_renovacao);
  ...
  return {
    ...
    cancelamento_efetivo_em: dataCancelamentoEfetivo.toISOString().split('T')[0],
    motivo: motivo,
    mensagem: `Sua assinatura continuará ativa até ${dataCancelamentoEfetivo.toISOString().split('T')[0]}`
  };
}
