...
const { planoDoTenant } = require('./planos');
  ...
  async function executarVerificacao() {
    try {
      ...
      for (const t of tenants) {
        // Plano interno é uso próprio do dono: não tem assinatura no Stripe, logo
        // não tem data de renovação. Sem esta guarda, a cobrança o veria como
        // "vencido desde sempre" e bloquearia a conta do dono.
        if (planoDoTenant(t.id) === 'interno') continue;
