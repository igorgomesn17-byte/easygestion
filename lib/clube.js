...
//
// CONFERE antes de devolver, no molde de lib/sku.js gerarCodigoBarras(). Antes isto
// sorteava e gravava direto: uma colisao estourava o UNIQUE e, como este vale nasce
// DENTRO da transacao da venda, derrubava a VENDA INTEIRA na frente da cliente.
// O UNIQUE(tenant_id, codigo) (migration 036) da a cada loja seu proprio espaco, e a
// checagem aqui tira o resto do risco. O UNIQUE continua sendo a garantia final —
// isto so evita o erro no caminho normal.
//
// Esta funcao e' a UNICA: routes/trocas.js tinha uma copia identica e agora importa
// daqui. Duas geradoras divergem com o tempo; uma so, nao.
function gerarCodigoVale(tenantId, tentativas = 12) {
  const t = exigirTenant(tenantId);
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  const jaExiste = db.prepare('SELECT 1 FROM vales WHERE codigo = ? AND tenant_id = ?');
  for (let i = 0; i < tentativas; i++) {
    let codigo = 'VALE-';
    for (let c = 0; c < 6; c++) codigo += chars.charAt(Math.floor(Math.random() * chars.length));
    if (!jaExiste.get(codigo, t)) return codigo;
  }
  throw new Error('Não foi possível gerar um código de vale único. Tente novamente.');
}
...
function emitirPremioClube(tenantId, clienteId, vendaId) {
  ...
  const codigo = gerarCodigoVale(t);
